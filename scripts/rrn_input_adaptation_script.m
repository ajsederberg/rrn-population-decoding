function sim_name = rrn_input_adaptation_script(network_code, pulse_width, rng_index, efficient_save)
% THIS IS NOT FUNCTIONAL AS OF 11/23/18 
...
stim_param.amp_range = logspace(log10(0.01), log10(2), 15);
...
%%%%%%%%%%%%%%%%% adaptation options
stim_param.ampfreq_combo = 'adaptsweep';
stim_param.tau_adapt = 18;  % units of tau: 350 ms. 
%%%%%%%%%%%%%%%%%
...
amp_high = min(5*amp_low, input_params_prelim.amp_range(end));



%% then repeat trials to make a smooth average evoke FR surface. 
stim_param.freq_range = linspace(0.1, 0.5, 5);  % now sample five points

stim_param.amp_range = linspace(amp_low, amp_high, 6);
stim_param.num_reps = 10;
...
sim_name = [network_param.network_tag '_adapt_pw' num2str(round(100*pulse_width))];
