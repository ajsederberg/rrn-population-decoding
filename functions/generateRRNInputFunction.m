function [input_fun, input_times, input_amplitudes, t0tf, stim_param] = generateRRNInputFunction(stim_param)
...
% thihs will not be used for any simulation except 'spEnoise': it is a
% vector of variances (sigma^2 ) for noise on each unit.
%%%%%%% THESE NUMBERS SHOULD BE SET AUTOMATICALLY, SOMEWHERE ELSE. FOR NOW
%%%%%%% THEY ARE HERE. 
% sig_min = 0.25;
% sig_range = 1;
% stim_param.sig2 = (sig_min + sig_range*rand(stim_param.numNeur, 1)).^2;
%%% revised, 2/19/20: low-d noise, temporal autocorrelation: this is the
%%% weight of connection between hidden latent units
num_noise_terms = 3;
sig_range = 1.6/num_noise_terms;
sig_min = 0;
stim_param.sig2 = (sig_min + sig_range*randn(stim_param.numNeur, num_noise_terms));
