...
% % cells. 
% % Saved backup - vPre20190820 without helper function. 
% 
...
% cycle through cell groups (exc50, inh50 are draws of just 50 e-cells or
% i-cells; 'b' is a second draw. Ideally would want many replicates but
% don't want to spend the time now. 
cellGroup_string = {'all', 'exc', 'excISM', 'inh', 'exc50', 'inh50', 'exc50b', 'inh50b'};
cls_res = cell(size(cellGroup_string));
for i_group = 1:length(cellGroup_string)

    cls_res{i_group} = fitMultiClassifiers_helper(activity_struct, cellGroup_string{i_group}, classifier_options);
end
output_filename = ['new' output_filename];
saveInParfor([eff_save_dir output_filename], ...
    classifier_options, cls_res)
% saveInParfor([full_save_dir output_filename], ...
%     train_inds, test_inds, activity_struct, classifier_options, ...
%     cls_res)