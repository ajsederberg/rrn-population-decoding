% plot some single-trial firing rates examples; runs eigenvector analysis. 
...
for i_rng = rng_list
file_tag = ['spnormE_g200_n500_ei100_rng' num2str(i_rng)];
% file_tag = ['spnormEL_g200_n500_ei100_rng' num2str(i_rng)]; 

% file_tag = ['spnorm_g400_n500_ei100_rng' num2str(i_rng)];


expsim_tag = 'f2fdiscExp_pw50';
load([results_dir file_tag '/matfiles/sim_data_' ...
    file_tag '_f2fdiscExp_pw50.mat']);

sel_info = load([results_dir file_tag '/matfiles/selXCJ_stats_' ...
    file_tag expsim_tag]);
%%
line_cmap = lines(4);
...
print(gcf, '-dpdf', ['plots/talk_plots/single_cell_raster_ex' num2str(ind_cell) '_' file_tag])
print(gcf, '-dsvg', ['plots/talk_plots/single_cell_raster_ex' num2str(ind_cell) '_' file_tag])
...
%% compute linearized J at the baseline values
b = network_param.b;
...
% linV is the linearized connectivity at the baseline fixed point
inp_patt_transformed = linV\network_param.input_pattern;
...
% use linD, linV : linearized at bbaseline fixed point
is_real = abs(imag(diag(linD))) < 1e-16;
real_evects = linV(:, is_real);
...
for i_vect = 1:10
i_ctr = i_ctr + 1;
    if linD(i_ctr, i_ctr) == conj(linD(i_ctr+1, i_ctr+1))
        % take pair of eigenvectors
        dr_v1 = reshape(dr_out_all*linV(:, i_ctr), nTimes, nStim);
        ...
        dr_out_vec_proj = reshape(dr_out_all*linV(:, i_ctr), nTimes, nStim);
...
for i_sp = 1:length(pc_wts_stim1)
    ...
    title(['\lambda = ' num2str(linD(i_sp, i_sp))])
    
    
    subplot(length(pc_wts_stim1), 3, 3*i_sp)
    if ~isreal(linV(:, i_sp))
        if mod(i_sp, 2) == 1
            plot(sel_info.cell_auroc_val, real(linV(:, i_sp)), '.')
            xc_val = corr(sel_info.cell_auroc_val, real(linV(:, i_sp)));
        else
            plot(sel_info.cell_auroc_val, imag(linV(:, i_sp)), '.')
            xc_val = corr(sel_info.cell_auroc_val, imag(linV(:, i_sp)));

        end
        

    else
        plot(sel_info.cell_auroc_val, (linV(:, i_sp)), '.')
        xc_val = corr(sel_info.cell_auroc_val, (linV(:, i_sp)));
...
for i_v = 1:size(linV, 2)
    if ~isreal(linD(i_v, i_v))
        if mod(i_v, 2) == 1
            [xc_val, p] = corr(sel_info.cell_auroc_val, real(linV(:, i_v)));
            xc_val = abs(xc_val);   % don't know if we took +/- imag part

        else
            [xc_val, p] = corr(sel_info.cell_auroc_val, imag(linV(:, i_v)));
            xc_val = abs(xc_val);   % don't know if we took +/- imag part
        ...
        [xc_val, p] = corr(sel_info.cell_auroc_val, (linV(:, i_v)));
...
dens_plot = histcounts2(sel_evec_linear_corr, real(diag(linD)), c_bins, L_bins);
...
subplot(222)
cell_auroc_val = sel_info.cell_auroc_val;
...
for i_group = 1:(length(evec_groups)-1)
    plot(real(e_vals(evec_bin_locs == i_group)), ...
        imag(e_vals(evec_bin_locs == i_group)), '.', 'color', cmap(i_group, :), ...
        'markersize', 10);
end
set(gca, 'color', 'none')
axis square
...
for i_group = 1:(length(inp_groups)-1)
    plot(real(e_vals(inp_bin_locs == i_group)), ... 
        imag(e_vals(inp_bin_locs == i_group)), '.', 'color', cmap(i_group, :), ...
        'markersize', 10);
end
set(gca, 'color', 'none')
axis square
...
print(gcf, '-dpdf', [results_dir file_tag '/plots/eigenvecs_' file_tag ...
    '_' expsim_tag]);

save([results_dir file_tag '/matfiles/eigenvectoranalysis_' ...
    file_tag '_' expsim_tag], 'e_vals', 'inp_pat_var', ...
    'sel_evec_linear_corr','xc_thresh', 'sel_evec_linear_corr', ...
    'sel_evec_linear_pcorr', 'D','linD', 'linV', 'cell_auroc_val', 'inp_patt_transformed')
% plotEvecAnalysis(file_tag)
